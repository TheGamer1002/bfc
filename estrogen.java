...
// Adapted from GeeksForGeeks "BrainFuck Interpreter in Java" (https://www.geeksforgeeks.org/brainfuck-interpreter-java/)
class estrogen {
  ...
  private static final String MOVE_PTR_RIGHT = "estrogen";
  private static final String MOVE_PTR_LEFT = "testosterone";
  private static final String INCREMENT = "progesterone";
  private static final String DECREMENT = "hrt";
  private static final String OUTPUT = "transfem";
  private static final String INPUT = "transmasc";
  private static final String LOOP_START = "blahaj";
  private static final String LOOP_END = ":3";
  ...
  private static void interpret(String[] s) {
    ...
    for (int i = 0; i < s.length; i++) {
      ...
      else if (s[i].equals(MOVE_PTR_LEFT)) {
      ...
      else if (s[i].equals(INCREMENT))
        memory[ptr]++;

        // - decrements the value of the memory cell
        // under the pointer
      else if (s[i].equals(DECREMENT))
        memory[ptr]--;

        // . outputs the character signified by the
        // cell at the pointer
      else if (s[i].equals(OUTPUT))
      ...
      else if (s[i].equals(LOOP_START)) {
        if (memory[ptr] == 0) {
          i++;
          while (c > 0 || !s[i].equals(LOOP_END)) {
            if (s[i].equals(LOOP_START))
              c++;
            else if (s[i].equals(LOOP_END))
              c--;
            i++;
          }
        }
      ...
      // cell under the pointer is nonzero
        ...
        if (memory[ptr] != 0) {
          i--;
          while (c > 0 || !s[i].equals(LOOP_START)) {
            if (s[i].equals(LOOP_END))
              c++;
            else if (s[i].equals(LOOP_START))
              c--;
            i--;
